...
# Stable substring of the attribution line (line 1 of every header), regardless
# of a source's comment_prefix. The provenance token sits on the line after it.
_ATTRIBUTION_MARKER = "Generated by cobo ("
# The provenance token: ``<source>/<name>@<short-or-full-sha>``. Names can hold
# ``+``/``#`` (C++, C#) but never ``/`` or whitespace.
_PROVENANCE_RE = re.compile(r"(?P<source>[^\s/]+)/(?P<name>[^@\s/]+)@[0-9a-f]{7,64}")


def parse_provenance(content: str) -> list[tuple[str, str]]:
    """Recover ``(source, name)`` pairs from a file's cobo provenance headers.

    Anchors on the attribution line and reads the token from the following line,
    so arbitrary file content that happens to look like a token is not misread.

    Args:
        content: The full text of a previously dumped file.

    Returns:
        Unique ``(source, name)`` pairs in first-seen order; empty when the file
        carries no recognizable cobo header.
    """
    lines = content.splitlines()
    pairs: list[tuple[str, str]] = []
    seen: set[tuple[str, str]] = set()
    for index, line in enumerate(lines):
        if _ATTRIBUTION_MARKER not in line or index + 1 >= len(lines):
            continue
        match = _PROVENANCE_RE.search(lines[index + 1])
        if match is None:
            continue
